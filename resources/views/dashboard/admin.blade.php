<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-6 lg:px-8 py-12 lg:py-16">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-8">
            <div>
                <h1 class="text-4xl font-display font-black text-slate-900 tracking-tighter flex items-center gap-4">
            ...
            <div class="relative w-full md:w-96 group">
                ...
                <input type="text" placeholder="Search system resources..." class="block w-full pl-12 pr-16 py-4 bg-white shadow-sm border border-slate-200 shadow-slate-200/50 rounded-2xl text-slate-300 placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-brand-500 focus:border-brand-500 transition-all text-sm font-bold uppercase tracking-widest">
                <div class="absolute inset-y-0 right-0 pr-4 flex items-center">
                    <kbd class="inline-flex items-center border border-slate-200 rounded-lg px-2 py-1 text-[10px] font-black text-slate-500 bg-slate-100 border border-slate-200 shadow-inner uppercase tracking-widest">Ctrl K</kbd>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8 mb-8">
            <div class="lg:col-span-3 bg-white shadow-sm border border-slate-200 shadow-slate-200/50 rounded-[2.5rem] p-8 flex flex-wrap md:flex-nowrap items-center gap-8 group hover:border-brand-500/30 transition-colors">
                ...
                <div class="flex-1 min-w-[150px]">
                    <div class="flex justify-between text-[10px] font-black uppercase tracking-widest mb-3"><span class="text-slate-500">CPU Load</span><span class="text-slate-900">24%</span></div>
                    <div class="w-full bg-slate-100 border border-slate-200 shadow-inner rounded-full h-1.5"><div class="bg-brand-500 h-1.5 rounded-full" style="width: 24%"></div></div>
                </div>
                
                <div class="flex-1 min-w-[150px]">
                    <div class="flex justify-between text-[10px] font-black uppercase tracking-widest mb-3"><span class="text-slate-500">Node Memory</span><span class="text-slate-900">4.2 / 16 GB</span></div>
                    <div class="w-full bg-slate-100 border border-slate-200 shadow-inner rounded-full h-1.5"><div class="bg-brand-500 h-1.5 rounded-full" style="width: 26%"></div></div>
                </div>
            </div>

            <div class="bg-white shadow-sm border border-slate-200 shadow-slate-200/50 rounded-[2.5rem] p-8 flex flex-col justify-center gap-4 group hover:border-brand-500/30 transition-colors">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
            ...
            @foreach($metrics as $metric)
                <div class="bg-white shadow-sm border border-slate-200 shadow-slate-200/50 rounded-[2.5rem] p-8 relative overflow-hidden group hover:border-brand-500/30 transition-colors">
                    <p class="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em] mb-4">{{ $metric['label'] }}</p>
                    <div class="flex items-end gap-3 mb-2">
                        <h3 class="text-4xl font-black text-slate-900 tracking-tighter">{{ $metric['value'] }}</h3>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
            <div class="lg:col-span-2 bg-white shadow-sm border border-slate-200 shadow-slate-200/50 rounded-[3rem] p-10 flex flex-col min-h-[450px] group hover:border-brand-500/30 transition-colors">
                <div class="flex justify-between items-center mb-10">
                    <div>
                        <h3 class="text-xl font-bold text-slate-900 mb-1">Financial Trajectory</h3>
            ...
            <div class="bg-white shadow-sm border border-slate-200 shadow-slate-200/50 rounded-[3rem] p-10 flex flex-col justify-between min-h-[450px] group hover:border-brand-500/30 transition-colors">
                <h3 class="text-xl font-bold text-slate-900 mb-8">User Topology</h3>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {{-- Approvals --}}
            <div class="bg-white shadow-sm border border-slate-200 shadow-slate-200/50 rounded-[3rem] p-10 group hover:border-brand-500/30 transition-colors">
                <div class="flex justify-between items-center mb-10">
                    <h3 class="text-xl font-bold text-slate-900">Governance</h3>
                ...
                <div class="space-y-4">
                    @forelse($pendingApprovals as $user)
                        <div class="bg-slate-100 border border-slate-200 shadow-inner rounded-[1.5rem] p-6 border border-white/5 flex justify-between items-center group/item hover:bg-white/10 transition-colors">
            ...
            {{-- Activity --}}
            <div class="bg-white shadow-sm border border-slate-200 shadow-slate-200/50 rounded-[3rem] p-10 group hover:border-brand-500/30 transition-colors">
                <h3 class="text-xl font-bold text-slate-900 mb-10 flex items-center gap-3">
                ...
                <div class="relative border-l-2 border-white/5 ml-2 space-y-8">
                    ...
                    @foreach($activities as $act)
                        <div class="relative pl-8">
                            ...
                            <p class="text-sm font-bold text-slate-900 leading-tight">{{ $act['msg'] }}</p>
            ...
            <div class="bg-white shadow-sm border border-slate-200 shadow-slate-200/50 rounded-[3rem] p-10 group hover:border-brand-500/30 transition-colors">
                <div class="flex justify-between items-center mb-10">
                    <h3 class="text-xl font-bold text-slate-900">Domain Focus</h3>
                    ...
                    @foreach($categories as $category)
                        <div class="group/cat cursor-pointer">
                            <div class="flex justify-between mb-2">
                                <h4 class="text-sm font-bold text-slate-900 group-hover/cat:text-brand-500 transition-colors">{{ $category->name }}</h4>
                            ...
                            <div class="w-full bg-slate-100 border border-slate-200 shadow-inner rounded-full h-1">
