    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('dashboard') }}"
               class="text-gray-400 hover:text-gray-600 transition">← Back to Dashboard</a>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Edit Session — <span class="text-sky-600">{{ $liveSession->title }}</span>
            </h2>
        </div>
    </x-slot>
                ...
                <form id="update-session-form" action="{{ route('instructor.live-sessions.update', $liveSession) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4"
                                  class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-sky-400 resize-y @error('description') border-red-400 @enderror">{{ old('description', $liveSession->description) }}</textarea>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="number" id="max_duration_minutes" name="max_duration_minutes" 
                                   ...
                                   class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-sky-400 @error('max_duration_minutes') border-red-400 @enderror"
                                   required>
                            @error('max_duration_minutes') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                    </div>
