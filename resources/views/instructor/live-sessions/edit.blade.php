    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('dashboard') }}"
               class="text-gray-600 hover:text-gray-900 transition">← Back to Dashboard</a>
            <h2 class="font-semibold text-xl text-gray-900 leading-tight">
                Edit Session — <span class="text-violet-600">{{ $liveSession->title }}</span>
            </h2>
        </div>
    </x-slot>
                ...
                <form id="update-session-form" action="{{ route('instructor.live-sessions.update', $liveSession) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4"
                                  class="w-full px-4 py-2.5 border border-gray-300 bg-white text-gray-900 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-600 focus:border-violet-600 resize-y @error('description') border-red-500 @enderror">{{ old('description', $liveSession->description) }}</textarea>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label for="scheduled_at" class="block text-sm font-semibold text-gray-900 mb-1.5">
                                Scheduled Date & Time <span class="text-red-600">*</span>
                            </label>
                        ...
                        <div>
                            <label for="max_duration_minutes" class="block text-sm font-semibold text-gray-900 mb-1.5">
                                Estimated Duration (minutes) <span class="text-red-600">*</span>
                            </label>
                            <input type="number" id="max_duration_minutes" name="max_duration_minutes" 
                                   ...
                                   class="w-full px-4 py-2.5 border border-gray-300 bg-white text-gray-900 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-600 focus:border-violet-600 @error('max_duration_minutes') border-red-500 @enderror"
                                   required>
                            @error('max_duration_minutes') <p class="text-red-600 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between pt-4 border-t border-gray-200">
                        ...
                        <div class="flex items-center gap-4">
                            <a href="{{ route('dashboard') }}"
                               class="px-5 py-2.5 text-sm text-gray-700 hover:text-gray-900 transition">Cancel</a>
                            <button type="submit"
                                    class="px-6 py-2.5 bg-violet-600 hover:bg-violet-700 text-white text-sm font-semibold rounded-lg transition">
                                Save Changes
                            </button>
                        </div>
                    </div>
