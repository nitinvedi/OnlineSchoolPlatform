<x-app-layout>
    <div x-data="{ videoModalOpen: false, activeTab: 'overview', expandedSection: null }" class="min-h-screen bg-[#F8FAFC]">
        ...
        {{-- BREADCRUMB --}}
        <div class="border-b border-slate-200 bg-[#F8FAFC] sticky top-0 z-40">
            <div class="max-w-full px-4 sm:px-8 lg:px-12 py-3">
                <nav class="text-[10px] font-mono uppercase tracking-widest text-gray-500" aria-label="Breadcrumb">
                    <ol class="flex items-center gap-2 flex-wrap">
                        ...
                        <li>·</li>
                        <li><a href="#" class="hover:text-[#0F172A] transition-colors">{{ $course->category->name ?? 'Category' }}</a></li>
                        <li>·</li>
                        <li class="text-[#0F172A] break-words">{{ $course->title }}</li>
                    </ol>
                </nav>
            </div>
        </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-[65%_1fr] gap-8 lg:gap-12">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap items-center gap-3 mb-6">
                            <span class="px-3 py-1 border border-[#333] text-[9px] font-mono uppercase tracking-widest text-[#0F172A]">
                        ...
                        {{-- Title --}}
                        <h1 class="text-[5vw] font-display font-black text-[#0F172A] leading-[1] mb-4">
                            {{ $course->title }}
                        </h1>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-sm font-sans text-[#0F172A]">By {{ $course->instructor->name ?? 'Expert Instructor' }}</span>
                        </div>
                    </div>
                    ...
                    <div class="hidden lg:block">
                        <div class="sticky top-24 space-y-4">
                            ...
                            <div class="bg-white border border-slate-200 overflow-hidden group cursor-pointer" @click="videoModalOpen = true">
                                <div class="relative aspect-video bg-gradient-to-br from-[#2255FF]/10 to-white overflow-hidden">
                            ...
                            <div class="bg-white border border-slate-200 p-3 space-y-1">
                                <p class="text-[9px] font-mono uppercase tracking-widest text-gray-600">Ends In</p>
                            ...
                            @if($enrolled)
                                <a href="{{ route('lessons.show', [$course, $course->lessons->first()]) }}" 
                                   class="block w-full h-13 bg-[#2255FF] text-[#0F172A] text-center line-clamp-1 leading-13 font-bold uppercase text-sm tracking-widest hover:bg-[#1a3fb3] transition-colors">
                                    Continue Learning
                                </a>
                            @else
                                <form action="{{ $course->price && $course->price > 0 ? route('payments.checkout', $course) : route('courses.enroll', $course) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="w-full h-13 bg-[#2255FF] text-[#0F172A] font-bold uppercase text-sm tracking-widest hover:bg-[#1a3fb3] transition-colors">
                            ...
                            {{-- Wishlist Button --}}
                            <button class="w-full h-11 border border-slate-200 text-gray-700 font-bold uppercase text-xs tracking-widest hover:bg-gray-50 transition-colors flex items-center justify-center gap-2">
                            ...
                            <div class="border-t border-slate-200 pt-3">
                                <p class="text-[9px] font-mono uppercase tracking-widest text-slate-500 mb-2.5">Share:</p>
                                <div class="flex gap-2">
                                    <a href="#" class="w-8 h-8 border border-slate-200 flex items-center justify-center text-slate-500 hover:border-[#2255FF] hover:text-[#2255FF] transition-colors">
        ...
        <div class="fixed bottom-0 left-0 right-0 lg:hidden bg-white border-t border-slate-200 p-3 flex gap-2 z-50">
            <div class="text-base font-display font-black text-gray-900">
                @if($course->price && $course->price > 0)
                    ${{ $course->price }}
                @else
                    FREE
            ...
            <form action="{{ $course->price && $course->price > 0 ? route('payments.checkout', $course) : route('courses.enroll', $course) }}" method="POST" class="flex-1">
                @csrf
                <button type="submit" class="w-full h-11 bg-violet-600 text-white font-bold uppercase text-xs tracking-widest hover:bg-violet-500 transition-colors">
            ...
            <div class="max-w-full px-4 sm:px-8 lg:px-12 py-16">
                ...
                <div x-show="activeTab === 'curriculum'" style="display: none;">
                    <div class="mb-16">
                        ...
                        <div class="space-y-0 border border-slate-200">
                            @php
                            ...
                            @foreach($sections as $sectionIndex => $lessonGroup)
                                <div class="border-b border-slate-200 last:border-b-0">
                                    ...
                                    <button @click="expandedSection === {{ $loop->index }} ? expandedSection = null : expandedSection = {{ $loop->index }}"
                                        ...
                                        <div class="flex items-center gap-3 text-[12px] font-mono uppercase tracking-widest text-[#0F172A]">
                                        ...
                                        @foreach($lessonGroup as $lesson)
                                            @if($enrolled)
                                                <a href="{{ route('lessons.show', [$course, $lesson]) }}" 
                                                   class="group flex items-center h-11 px-6 border-t border-slate-200 hover:bg-[#161616] transition-colors">
                                                    ...
                                                    <div class="flex items-center gap-4 ml-4 flex-shrink-0">
                                                        <span class="text-[10px] font-mono uppercase tracking-widest text-slate-500">{{ $lesson->duration_minutes ?? 12 }} MIN</span>
                                            ...
                                            @else
                                                <div class="flex items-center h-11 px-6 border-t border-slate-200 opacity-50">
                                                    <div class="flex items-center gap-3 flex-1 min-w-0">
                                                        <svg class="w-5 h-5 text-slate-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>
                                                        <span class="text-[13px] font-sans text-[#0F172A] truncate">{{ $lesson->title }}</span>
                                                    </div>
                                                    <span class="text-[10px] font-mono uppercase tracking-widest text-slate-500 ml-4">LOCKED</span>
                                                </div>
                                            @endif
                                        @endforeach
                                    </div>
                        ...
                        <div class="flex flex-col md:flex-row gap-8">
                            <div class="flex-shrink-0">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode($course->instructor->name ?? 'Instructor') }}&background=2255FF&color=fff&size=160" 
                                     alt="{{ $course->instructor->name ?? 'Instructor' }}" 
                                     class="w-20 h-20 rounded-lg border border-slate-200">
                            </div>
                            <div class="flex-1">
                                <h3 class="text-2xl font-display font-black text-[#0F172A] mb-2">{{ $course->instructor->name ?? 'Expert Instructor' }}</h3>
                                <p class="text-[11px] font-mono uppercase tracking-widest text-slate-500 mb-6">Senior Industry Expert</p>
                                ...
                                <div class="grid grid-cols-4 gap-4 mb-6">
                                    <div class="text-center">
                                        <div class="text-[24px] font-display font-black text-[#0F172A] mb-1">15K+</div>
                                        <p class="text-[10px] font-mono uppercase tracking-widest text-slate-500">Students</p>
                                    </div>
                                    <div class="text-center">
                                        <div class="text-[24px] font-display font-black text-[#0F172A] mb-1">4.9★</div>
                                        <p class="text-[10px] font-mono uppercase tracking-widest text-slate-500">Rating</p>
                                    </div>
                                    <div class="text-center">
                                        <div class="text-[24px] font-display font-black text-[#0F172A] mb-1">12</div>
                                        <p class="text-[10px] font-mono uppercase tracking-widest text-slate-500">Courses</p>
                                    </div>
                                    <div class="text-center">
                                        <div class="text-[24px] font-display font-black text-[#0F172A] mb-1">15y</div>
                                        <p class="text-[10px] font-mono uppercase tracking-widest text-slate-500">Experience</p>
                                    </div>
                                </div>

                                <div class="flex gap-3">
                                    <a href="#" class="w-10 h-10 border border-slate-200 flex items-center justify-center text-slate-500 hover:border-[#2255FF] hover:text-[#2255FF] transition-colors rounded">
                    ...
                    <div class="mb-16">
                        ...
                        <div class="flex flex-col md:flex-row gap-12 mb-12">
                            <div class="flex flex-col items-center">
                                <div class="text-[6vw] font-display font-black text-[#0F172A]">{{ number_format($course->rating ?? 4.9, 1) }}</div>
                                <div class="flex gap-1.5 text-[#F5A623] my-3">
                                    @for($i = 0; $i < 5; $i++)
                                        <svg class="w-5 h-5 fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                    @endfor
                                </div>
                                <p class="text-[10px] font-mono uppercase tracking-widest text-slate-500">AVERAGE RATING</p>
                            </div>
                            ...
                            <div class="flex-1 space-y-3">
                                @foreach([5 => 78, 4 => 15, 3 => 5, 2 => 1, 1 => 1] as $star => $pct)
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="text-[10px] font-mono uppercase tracking-widest text-slate-500 w-8 text-right">{{ $pct }}%</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
